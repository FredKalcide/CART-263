//OOP exercise
/*
let ball = []; //putting every particle in an array

function setup(){
    createCanvas(400, 400);
    for (let i = 0; i < 50; i++){ // bringing 50 particles into existence in the array, aka running the class 50 times
        ball[i] = new Particle();
    }
}
...
function draw(){
    ...
    for(let i = 0; i < ball.length; i++){ // for loop to call our methods on each and every ball
        ball[i].particleActions();
        ball[i].collisions();
    }
}
...
class Particle{
    constructor() {
        ...
        this.speed = 2  // jitter speed, this number determines a greater or lesser range of motion for the random in the particleActions fx
    ...
    particleActions(){
        ...
        this.x += random(-this.speed, this.speed); // jitter on x
        this.y += random(-this.speed, this.speed); // jitter on y
